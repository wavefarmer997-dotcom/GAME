/**
 * NEXUS GAMING TH - Persistent Database Engine
 * File-based JSON Database with Atomic Writes, Collections, and Auto-Backup
 */

const fs = require('fs');
const path = require('path');

const DATA_DIR = path.join(__dirname, '..', 'data');
const DB_FILE = path.join(DATA_DIR, 'nexus_db.json');
const BACKUPS_DIR = path.join(DATA_DIR, 'backups');

class DatabaseEngine {
  constructor() {
    this.db = null;
    this.isWriting = false;
    this.writeQueue = [];
    this.init();
  }

  init() {
    try {
      if (!fs.existsSync(DATA_DIR)) {
        fs.mkdirSync(DATA_DIR, { recursive: true });
      }
      if (!fs.existsSync(BACKUPS_DIR)) {
        fs.mkdirSync(BACKUPS_DIR, { recursive: true });
      }

      if (fs.existsSync(DB_FILE)) {
        const raw = fs.readFileSync(DB_FILE, 'utf-8');
        this.db = JSON.parse(raw);
        console.log('✅ [Database] Loaded existing database from:', DB_FILE);
      } else {
        console.log('ℹ️ [Database] Initializing fresh database with default seed...');
        this.db = this._getSeedData();
        this._saveSync();
      }
    } catch (err) {
      console.error('❌ [Database] Error initializing database:', err);
      this.db = this._getSeedData();
    }
  }

  _getSeedData() {
    return {
      version: '2.0.0',
      lastUpdated: new Date().toISOString(),
      users: [
        {
          id: 'u-nexus',
          name: 'NexusGamerTH',
          gamerTag: '#7788',
          email: 'nexus@nexus.th',
          password: 'password123',
          bio: 'เกมเมอร์สายวาโล & แดนไอยาสิทธิ์ สตรีมเมอร์ฝึกหัด พร้อมหาตี้เล่นสนุก ไม่ Toxic ✨',
          avatar: 'https://images.unsplash.com/photo-1566492031773-4f4e44671857?auto=format&fit=crop&w=200&q=80',
          banner: 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&w=1200&q=80',
          level: 38,
          xp: 3850,
          xpNext: 5000,
          rankTitle: '🌟 Mythic Challenger',
          joinedDate: 'กุมภาพันธ์ 2025',
          favoriteGames: ['Valorant', 'Genshin Impact', 'Black Myth: Wukong'],
          friends: ['p-1', 'p-4'],
          friendRequests: ['p-3', 'p-9'],
          sentRequests: ['p-2'],
          following: ['p-1', 'p-6'],
          stats: {
            postsCount: 14,
            squadsJoined: 28,
            tournamentsEntered: 3,
            reputationScore: 98
          },
          badges: [
            { id: 'b1', name: '👑 Early Adopter', icon: '👑', desc: 'สมาชิกรุ่นบุกเบิก NEXUS TH' },
            { id: 'b2', name: '🎯 Clutch Master', icon: '🎯', desc: 'ชนะแมตช์หาตี้ติดต่อกัน 10 ครั้ง' },
            { id: 'b3', name: '💬 Top Contributor', icon: '💬', desc: 'เขียนไกด์ยอดนิยมที่มีคนกดไลก์เกิน 200+' },
            { id: 'b4', name: '🛡️ Verified Gamer', icon: '🛡️', desc: 'ยืนยันตัวตนและผูกบัญชีสำเร็จ' }
          ]
        },
        {
          id: 'u-neon',
          name: 'Agent_NeonX',
          gamerTag: '#5544',
          email: 'neon@nexus.th',
          password: 'password123',
          bio: 'Speed & Electricity ⚡ Radiant Duelist พร้อมลุยทุกแรงก์ สตรีมเมอร์พาร์ทไทม์',
          avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80',
          banner: 'https://images.unsplash.com/photo-1511512578047-dfb367046420?auto=format&fit=crop&w=1200&q=80',
          level: 50,
          xp: 5100,
          xpNext: 7500,
          rankTitle: '🌟 Immortal Legend',
          joinedDate: 'มกราคม 2025',
          favoriteGames: ['Valorant', 'Apex Legends'],
          friends: ['p-1', 'p-3', 'p-5'],
          friendRequests: ['p-4'],
          sentRequests: [],
          following: ['p-1'],
          stats: { postsCount: 22, squadsJoined: 45, tournamentsEntered: 6, reputationScore: 99 },
          badges: [
            { id: 'b-radiant', name: '⚡ Radiant Legend', icon: '⚡', desc: 'ผู้เล่นระดับท็อปของเซิร์ฟเวอร์' },
            { id: 'b-mvp', name: '🏆 Premier MVP', icon: '🏆', desc: 'คว้า MVP การแข่งขันประจำเดือน' }
          ]
        },
        {
          id: 'u-sova',
          name: 'Sova_ArrowKing',
          gamerTag: '#9999',
          email: 'sova@nexus.th',
          password: 'password123',
          bio: 'Radiant Initiator • Master of Lineups 🏹🎯 รู้ทุกจุดสแกน Recon และ Shock Dart',
          avatar: 'https://images.unsplash.com/photo-1527980965255-d3b416303d12?auto=format&fit=crop&w=200&q=80',
          banner: 'https://images.unsplash.com/photo-1579783902614-a3fb3927b675?auto=format&fit=crop&w=1200&q=80',
          level: 45,
          xp: 4200,
          xpNext: 6000,
          rankTitle: '👑 Radiant Champion',
          joinedDate: 'ธันวาคม 2024',
          favoriteGames: ['Valorant', 'Black Myth: Wukong'],
          friends: ['p-2', 'p-4'],
          friendRequests: [],
          sentRequests: [],
          following: ['p-2'],
          stats: { postsCount: 18, squadsJoined: 32, tournamentsEntered: 4, reputationScore: 97 },
          badges: [{ id: 'b-lineup', name: '🏹 Lineup God', icon: '🏹', desc: 'ผู้เชี่ยวชาญการเปิดวิสัยทัศน์ในแมพ' }]
        }
      ],
      posts: [
        {
          id: 'post-1',
          title: '🔥 สรุป Meta Patch ล่าสุด Valorant: ทำไม Vyse ถึงเปลี่ยนการเล่นแผนที่ Haven ไปตลอดกาล!',
          content: 'การมาถึงของ Agent สาย Sentinel คนล่าสุดอย่าง Vyse ทำให้การบุก A Long และ C Garage ใน Haven กลายเป็นเรื่องยากมาก Arc Rose สามารถแฟลชทะลุกำแพงได้แบบไม่ให้ศัตรูตั้งตัว แถมยังมี Shear ดักทางไม่ให้ Retake ได้ง่ายๆ แนะนำคอมโบกับ Sova หรือ Breach สบายๆ เลยครับ',
          author: {
            id: 'u-sova',
            name: 'Sova_ArrowKing',
            tag: '@arrow_master',
            avatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80',
            badge: '👑 Radiant Player',
            level: 45,
            verified: true
          },
          gameId: 'valorant',
          gameName: 'Valorant',
          category: 'guides',
          tags: ['Valorant', 'MetaAnalysis', 'Vyse', 'HavenMap', 'Tactics'],
          image: 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&w=800&q=80',
          mediaType: 'image',
          video: null,
          upvotes: 342,
          downvotes: 12,
          userVoted: 1,
          commentsCount: 48,
          views: 3120,
          createdAt: '10 นาทีที่แล้ว',
          pinned: true,
          comments: [
            {
              id: 'c-1',
              author: {
                name: 'Agent_NeonX',
                tag: '@neon_aimbot',
                avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&q=80',
                level: 32
              },
              content: 'เห็นด้วยเลยครับ! โดนกำแพงหนามสกัดตอนสไลด์เข้าไปนี่แทบกรี๊ด ต้องแก้เกมด้วยการให้ Initiator สแกนเคลียร์ก่อนเสมอ',
              upvotes: 28,
              createdAt: '5 นาทีที่แล้ว'
            }
          ]
        },
        {
          id: 'post-2',
          title: '🌋 เจาะลึกการบิลด์ Mavuika เทพอัคคีแห่ง Natlan: อาวุธ อาร์ติแฟกต์ และทีมที่ดีที่สุด!',
          content: 'Mavuika เป็นตัวละครที่มีความยืดหยุ่นสูงมาก สามารถเล่นได้ทั้ง Main DPS บนสนามและ Sub-DPS บัฟดาเมจธาตุไฟให้กับทีมได้อย่างมหาศาล',
          author: {
            id: 'u-mavuika',
            name: 'MavuikaSimp',
            tag: '@natlan_hero',
            avatar: 'https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?auto=format&fit=crop&w=150&q=80',
            badge: '🔥 Theorycrafter',
            level: 60,
            verified: true
          },
          gameId: 'genshin',
          gameName: 'Genshin Impact',
          category: 'guides',
          tags: ['GenshinImpact', 'Natlan', 'Mavuika', 'BuildGuide'],
          image: 'https://images.unsplash.com/photo-1578632767115-351597cf2477?auto=format&fit=crop&w=800&q=80',
          mediaType: 'image',
          video: null,
          upvotes: 488,
          downvotes: 6,
          userVoted: 0,
          commentsCount: 65,
          views: 5240,
          createdAt: '1 ชั่วโมงที่แล้ว',
          pinned: false,
          comments: []
        },
        {
          id: 'post-3',
          title: '⚡ [คลิปไฮไลท์] จังหวะ 1v5 Ace Clutch พลิกเกมรอบชิง Diamond Rank!',
          content: 'เหลือคนเดียวกับเลือด 14 HP สเต็ปยิงหัวเรียงตัวแบบดึงสติสุดๆ กดดูคลิปกันได้เลยครับ!',
          author: {
            id: 'u-khaosan',
            name: 'Khaosan_Clutcher',
            tag: '@khaosan_pro',
            avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80',
            badge: '🏆 Tournament MVP',
            level: 52,
            verified: true
          },
          gameId: 'valorant',
          gameName: 'Valorant',
          category: 'highlights',
          tags: ['Valorant', 'Ace', '1v5Clutch', 'Highlights'],
          image: 'https://images.unsplash.com/photo-1511512578047-dfb367046420?auto=format&fit=crop&w=800&q=80',
          mediaType: 'video',
          video: 'https://media.w3.org/2010/05/sintel/trailer.mp4',
          upvotes: 512,
          downvotes: 4,
          userVoted: 0,
          commentsCount: 78,
          views: 4320,
          createdAt: '2 ชั่วโมงที่แล้ว',
          pinned: false,
          comments: []
        }
      ],
      squads: [
        {
          id: 'sq-1',
          gameId: 'valorant',
          gameName: 'Valorant',
          title: '🔥 ไต่ Diamond ไป Ascendant ขอคนมีไมค์ สื่อสารดี ไม่โยน',
          host: {
            id: 'u-1',
            name: 'Agent_NeonX',
            tag: '@neon_aimbot',
            avatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80',
            rank: 'Diamond 2',
            level: 32
          },
          mode: 'Competitive 5v5',
          rankRequired: 'Diamond 1 - Ascendant 1',
          hasMic: true,
          roomType: 'text',
          slotsNeeded: 2,
          maxSlots: 5,
          members: [
            { name: 'Agent_NeonX', role: 'Duelist (Neon/Jett)', rank: 'D2', isHost: true },
            { name: 'Sova_ArrowKing', role: 'Initiator (Sova/Fade)', rank: 'D3', isHost: false },
            { name: 'SmokeMaster_TH', role: 'Controller (Omen/Astra)', rank: 'D1', isHost: false }
          ],
          createdAt: '5 นาทีที่แล้ว',
          status: 'open'
        },
        {
          id: 'sq-2',
          gameId: 'genshin',
          gameName: 'Genshin Impact',
          title: '🌋 ฟาร์มบอส Natlan + ช่วยเก็บกล่อง 100% โลก 8 ยินดีช่วยผู้เล่นใหม่',
          host: {
            id: 'u-2',
            name: 'MavuikaSimp',
            tag: '@natlan_hero',
            avatar: 'https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?auto=format&fit=crop&w=150&q=80',
            rank: 'AR 60 (WL 9)',
            level: 60
          },
          mode: 'Co-Op Farm / Exploration',
          rankRequired: 'AR 55+',
          hasMic: false,
          roomType: 'text',
          slotsNeeded: 2,
          maxSlots: 4,
          members: [
            { name: 'MavuikaSimp', role: 'DPS / Guide', rank: 'AR 60', isHost: true },
            { name: 'FurinaFanboy', role: 'Healer / Sub', rank: 'AR 58', isHost: false }
          ],
          createdAt: '12 นาทีที่แล้ว',
          status: 'open'
        }
      ],
      tournaments: [
        {
          id: 'tour-1',
          title: '⚔️ NEXUS VALORANT PREMIER CUP 2026 Season 1',
          gameId: 'valorant',
          gameName: 'Valorant',
          banner: 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&w=1200&q=80',
          prizePool: '฿50,000',
          totalTeams: 32,
          registeredCount: 28,
          status: 'registering',
          regDeadline: '10 ก.ย. 2026',
          startDate: '15 ก.ย. 2026',
          format: '5v5 Single Elimination (Bo3 Finals)',
          rules: 'รับผู้เล่นแรงก์ Platinum ขึ้นไป ปฏิบัติตามกฎสากล มีทีมถ่ายทอดสดพากย์ไทย',
          featured: true,
          registeredTeams: [
            { id: 't-1', name: 'Shadow Strikers', leader: 'Agent_NeonX', members: 5, seed: 1, logo: '⚡' },
            { id: 't-2', name: 'Cyber Phantoms', leader: 'Khaosan_Clutcher', members: 5, seed: 2, logo: '👻' }
          ]
        }
      ],
      chatMessages: [
        {
          id: 'msg-1',
          channel: 'general',
          userId: 'u-1',
          userName: 'Agent_NeonX',
          userAvatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80',
          userLevel: 48,
          badge: '👑 Radiant Master',
          text: 'สวัสดีชาว NEXUS ทุกคนครับ! คืนนี้มีใครลงแข่งทัวร์นาเมนต์มั่ง ขอเสียงหน่อยยย ⚡🔥',
          time: '20:15',
          timestamp: Date.now() - 1200000,
          reactions: { '🔥': 8, '👑': 5 }
        },
        {
          id: 'msg-2',
          channel: 'general',
          userId: 'u-sova',
          userName: 'Sova_ArrowKing',
          userAvatar: 'https://images.unsplash.com/photo-1527980965255-d3b416303d12?auto=format&fit=crop&w=200&q=80',
          userLevel: 45,
          badge: '👑 Radiant Champion',
          text: 'ทีมผมซ้อมเตรียมพร้อมแล้ว เจอกันรอบ 8 ทีมครับผม!',
          time: '20:18',
          timestamp: Date.now() - 900000,
          reactions: { '🎯': 6, '❤️': 4 }
        }
      ],
      stories: [
        {
          id: 'story-self',
          userId: 'current-user',
          userName: 'NexusGamerTH',
          userTag: '#7788',
          userAvatar: 'https://images.unsplash.com/photo-1566492031773-4f4e44671857?auto=format&fit=crop&w=200&q=80',
          userLevel: 38,
          type: 'video',
          mediaUrl: 'https://media.w3.org/2010/05/sintel/trailer.mp4',
          caption: '🔥 วอร์มนิ้วก่อนลงแข่งทัวร์นาเมนต์รอบดึก คืนนี้ลุยเต็มที่!',
          tag: '#RankUp #ValorantTH #Challenger',
          badge: '🌟 Mythic Challenger',
          bgGradient: 'linear-gradient(135deg, #7928CA, #FF0080)',
          createdAt: '15 นาทีที่แล้ว',
          viewsCount: 142,
          reactions: { '🔥': 38, '👑': 14, '⚡': 22, '🎯': 9, '❤️': 27 }
        },
        {
          id: 'story-1',
          userId: 'u-1',
          userName: 'Agent_NeonX',
          userTag: '@neon_aimbot',
          userAvatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80',
          userLevel: 48,
          type: 'image',
          mediaUrl: 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&w=1080&q=80',
          caption: '⚡ 1v4 Retake A Site สบายๆ ไต่ขึ้น Radiant 400 RR แล้ว!',
          tag: '#Radiant #ClutchGod #NeonMain',
          badge: '👑 Radiant Master',
          bgGradient: 'linear-gradient(135deg, #0070F3, #00DFD8)',
          createdAt: '45 นาทีที่แล้ว',
          viewsCount: 380,
          reactions: { '🔥': 89, '👑': 52, '⚡': 64, '🎯': 30, '❤️': 45 }
        }
      ],
      reviews: [
        {
          id: 'rev-1',
          gameId: 'valorant',
          gameName: 'Valorant',
          rating: 5,
          title: 'เกมยิงปืน Competitive ที่ดีที่สุดแห่งยุค ระบบกันโปร Vanguard ทำงานยอดเยี่ยม',
          content: 'การเล่น 5v5 มีความลึกในเชิงกลยุทธ์สูงมาก การผสมผสานของสกิลและ Gunplay ทำออกมาได้ลงตัว เล่นกับเพื่อนคือสนุกที่สุดแล้ว',
          author: {
            name: 'Agent_NeonX',
            tag: '@neon_aimbot',
            avatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80',
            hoursPlayed: 1450
          },
          pros: ['กันโปรเก่ง', 'กราฟิกลื่นไหล', 'อัปเดตสม่ำเสมอ'],
          cons: ['บางครั้งเจอผู้เล่น Toxic', 'สกินปืนราคาค่อนข้างแพง'],
          upvotes: 124,
          createdAt: '1 วันที่แล้ว'
        }
      ]
    };
  }

  _saveSync() {
    try {
      this.db.lastUpdated = new Date().toISOString();
      const content = JSON.stringify(this.db, null, 2);
      
      // Atomic write using temp file
      const tmpFile = `${DB_FILE}.tmp`;
      fs.writeFileSync(tmpFile, content, 'utf-8');
      fs.renameSync(tmpFile, DB_FILE);
      
      this._createSnapshotBackup(content);
    } catch (err) {
      console.error('❌ [Database] Failed to write database to disk:', err);
    }
  }

  _createSnapshotBackup(content) {
    try {
      const now = new Date();
      const dateStr = now.toISOString().replace(/[:.]/g, '-');
      const backupPath = path.join(BACKUPS_DIR, `backup-${dateStr}.json`);
      fs.writeFileSync(backupPath, content, 'utf-8');

      // Keep only last 10 backups
      const files = fs.readdirSync(BACKUPS_DIR)
        .filter(f => f.startsWith('backup-') && f.endsWith('.json'))
        .map(f => path.join(BACKUPS_DIR, f))
        .sort((a, b) => fs.statSync(b).mtimeMs - fs.statSync(a).mtimeMs);

      if (files.length > 10) {
        files.slice(10).forEach(f => fs.unlinkSync(f));
      }
    } catch (e) {
      // Ignore backup error to prevent blocking main flow
    }
  }

  // --- Public CRUD Methods ---

  getAll() {
    return this.db;
  }

  getCollection(collectionName) {
    return this.db[collectionName] || [];
  }

  getItem(collectionName, id) {
    const list = this.getCollection(collectionName);
    return list.find(item => item.id === id) || null;
  }

  insertItem(collectionName, item) {
    if (!this.db[collectionName]) {
      this.db[collectionName] = [];
    }
    if (!item.id) {
      item.id = `${collectionName.slice(0, 3)}-${Date.now()}`;
    }
    if (!item.createdAt) {
      item.createdAt = 'เมื่อสักครู่';
    }
    this.db[collectionName].unshift(item);
    this._saveSync();
    return item;
  }

  updateItem(collectionName, id, updatesOrUpdater) {
    const list = this.getCollection(collectionName);
    const index = list.findIndex(item => item.id === id);
    if (index === -1) return null;

    if (typeof updatesOrUpdater === 'function') {
      list[index] = updatesOrUpdater(list[index]);
    } else {
      list[index] = { ...list[index], ...updatesOrUpdater };
    }

    this._saveSync();
    return list[index];
  }

  deleteItem(collectionName, id) {
    const list = this.getCollection(collectionName);
    const index = list.findIndex(item => item.id === id);
    if (index === -1) return false;

    const removed = list.splice(index, 1);
    this._saveSync();
    return removed[0] || true;
  }

  syncAll(incomingState) {
    if (!incomingState || typeof incomingState !== 'object') return false;
    
    // Merge collections
    ['posts', 'squads', 'tournaments', 'chatMessages', 'stories', 'reviews', 'users'].forEach(key => {
      if (Array.isArray(incomingState[key])) {
        this.db[key] = incomingState[key];
      }
    });

    this._saveSync();
    return true;
  }

  resetToDefault() {
    this.db = this._getSeedData();
    this._saveSync();
    return this.db;
  }
}

const dbInstance = new DatabaseEngine();
module.exports = dbInstance;
